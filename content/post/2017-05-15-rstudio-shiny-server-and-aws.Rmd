...
After realizing how fast I can burn through my free 25 hours on shinyapps.io, I decided to repurpose my RStudio Server to also work with Shiny Server. Here's my new setup:
...
Amazon is nice enough to provide 1 year's worth of access to their [Free Tier](https://aws.amazon.com/free/) for AWS. There are a huge number of options available, but the important one is that they provide a free 750 hours/month to deploy an EC2 instance. That's just enough to keep 1 EC2 instance active 24/7, since 24 hours x 31 days = 744.
...
```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/sign-in.png")
```

If all goes well, you'll be at the console.

```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/console.png")
```
...
You can find the link to the image [here](http://www.louisaslett.com/RStudio_AMI/). Click one of the links on the right to start the setup, I normally click the one closest to me regionally.

The Virginia link takes me here:

```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/aws-rstudio-link.png")
```
...
_Important!_ Amazon will pop up a message box that talks about a _key pair_. This is how you will SSH into your server later on. This is really important, as you only get this screen one time, and can never come back to it. Setup a new key pair name (it can be anything), and click "Download Key Pair."

```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/key-pair.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/ec2-dashboard.png")
```

Click on "1 Running Instance," and you'll see your server starting up. Below, it's the one that says "running."

```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/start-up.png")
```
...
```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/rstudio-login.png")
```

If it worked, you should see this.

```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/logged-in.png")
```

## Step 3: New password for RStudio Server

It's advised that you immediately change the password. There are two ways to do so. The first way is easy. In the `Welcome.R` file that is shown above, you'll see a description for how to `library("RStudioAMI")` and then run `passwd()`. You can do that, but eventually you'll have to SSH into your server for _something_, so you may as well learn how now.
...
```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/ssh.png")
```

I run on a Mac, so I'll be using Terminal. If you run on Windows, you'll need to download [PuTTY](http://www.putty.org/). Open up Terminal, and type in the following for step 3:
...
```{r echo=FALSE}
knitr::include_graphics("/img/rstudio-shiny-server-aws-1/ssh-login.png")
```
...
Finally, you'll update to the latest version of RStudio Server. At the time of writing, this is 1.0.143, but it updates regularly, so go [here](https://www.rstudio.com/products/rstudio/download-server/) and scroll down to find the latest update for 64bit Ubuntu. The commands generally look like:
