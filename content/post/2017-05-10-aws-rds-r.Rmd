...
* `drv` - The driver I used is from the `RMySQL` package, an implementation of the general interface provided by `DBI`. I'll leave it to [the experts](https://github.com/rstats-db/DBI) to explain all of this.
...
Select the apple table from the database. _This does not actually pull the data into memory_. It just makes a connection!
